...
class Inbox: NSObject {
    ...
    class func getInboxMessages(page : Int, count : Int, callback : ([Inbox]!, NSError!) -> Void) {
        let user = NSUserDefaults.standardUserDefaults().dictionaryForKey("user")!
        let user_id = user["id"] as! Int
        AbletiveAPIClient.sharedClient().GET("user/get_inbox", parameters: ["page" : page, "count" : count, "user_id" : user_id], success: { (dataTask, response) in
            let JSON = response as! NSDictionary
            if JSON["status"] as! String == "ok" {
                let messagesList = JSON["messages"] as! NSArray
                var inboxes = [Inbox]()
                for i in 0..<messagesList.count {
                    let inbox = Inbox(attribute: messagesList.objectAtIndex(i) as! Dictionary<String, AnyObject>)
