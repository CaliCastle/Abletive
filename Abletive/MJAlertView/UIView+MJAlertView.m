...
#define NOTIFIER_LABEL_FONT ([UIFont systemFontOfSize:18])
#define NOTIFIER_CANCEL_FONT ([UIFont systemFontOfSize:13])
...
static const CGFloat kHeightFromBottom = 140.f;
    ...
    if(notifierView) {
        ...
        [self updateNotifierWithAnimation:notifierView withText:text completion:^(BOOL finished) {
            ...
            if(!shouldDismiss) {
                ...
                [buttonCancel setTitle:@"知道啦" forState:UIControlStateNormal];
        ...
        switch (style) {
            case MJAlertViewError:
                notifierView.backgroundColor = UIColorFromRGB(0xF94137);
                break;
            case MJAlertViewSuccess:
                notifierView.backgroundColor = [AppColor mainYellow];
                break;
            case MJAlertViewWarning:
                notifierView.backgroundColor = [AppColor secondaryBlack];
                break;
            default:
                break;
        }
