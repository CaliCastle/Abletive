...
@property (strong, nonatomic) void (^sizeChangedHandler)(CGFloat);
...
- (void)loadUI{
    ...
    self.textView.layer.borderColor = [AppColor lightSeparator].CGColor;
    ...
    [self.sendButton setTitleColor:[AppColor mainYellow] forState:UIControlStateNormal];
    ...
    [self.sendButton addTarget:self action:@selector(sendTextCommentTapped:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)layout{
    ...
    if (self.sizeChangedHandler){
        self.sizeChangedHandler(addBarFrame.size.height);
    }
}
...
- (void)setInputBarSizeChangedHandle:(void (^)(CGFloat))handler{
...
- (void)sendTextCommentTapped:(UIButton *)button{
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text {
    if ([text isEqualToString:@"\n"]){
        [self sendTextCommentTapped:nil];
        return NO;
    }
    return YES;
}
